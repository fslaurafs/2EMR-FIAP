...
public class Biblioteca {
	...
	public void adiciona(Livro livro) {
		if (this.cheia()) System.err.println("Biblioteca com capacidade maxima atingida.");
		this.totalLivros++;
	...
	public Livro remove(Livro livro) {
		...
		for(int i = 0; i < this.totalLivros; i++) {
			if(livro.getTitulo().equalsIgnoreCase(this.livros[i].getTitulo()) && !removido) { //procurando o livro pra setar que ele foi removido
				livroRemovido = this.livros[i];
				removido = true;
			}
			
			if (removido && (i +1) < this.livros.length) { //fazer a movimentação e aproveitar o for
